...
class TopicData:
    ...
    def splitTime(self):
        for i in range(1, len(self.m_dict)+1):
            raw_time_input = 2
            temp_str = self.m_dict[i].pop(raw_time_input)

            #if the raw time is 'whole' --> split into '' and ''
            if temp_str == 'whole':
                self.m_dict[i].append('')
                self.m_dict[i].append('')
            ...
            #the raw time is split into start time and end time     
                ...
                for j in range(0,len(temp_list)):
                    if temp_list[j] == 'start' or temp_list[j] == 'end':
                        temp_list[j] = ''
                self.m_dict[i].append(temp_list[0])
                self.m_dict[i].append(temp_list[1])
...
wb1 = openpyxl.load_workbook("test01.xlsx")
...
class Output:
    ...
    """
	def __init__(self, head_file, body_file1, body_file2, end_file):
		tmp_head_file = open(head_file, 'r+')
		tmp_body_file01 = open(body_file1, 'r+')
		tmp_body_file02 = open(body_file2, 'r+')
		tmp_end_file = open(end_file,'r+')

		self.head_text0 = tmp_head_file.read()
		self.body_text01 = tmp_body_file01.read()
		self.body_text02 = tmp_body_file02.read()
		self.end_text0 = tmp_end_file.read()
		
		self.head_text = copy.deepcopy(self.head_text0)
		self.body_text = copy.deepcopy(self.body_text01)
...
template = Output("template_head.txt","template_body1.txt", "template_body2.txt" ,"template_end.txt")
template.export(2)
#template.modify_HeadSlidesN(2)
#template.modify_HeadSegment(2)
